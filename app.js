...
(function () {
  ...
  if (window.__nytrip_inited) {
    console.log("NY TRIP: ya inicializado. Abortando carga duplicada.");
    return;
  }
  window.__nytrip_inited = true;
  ...
  async function initApp() {
    console.log("initApp arrancando. supabaseApp:", supabaseApp);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // Registrar handler de login UNA sola vez
    if (!window.__nytrip_loginHandlerRegistered) {
      window.__nytrip_loginHandlerRegistered = true;
        ...
        loginForm.addEventListener("submit", async (e) => {
          ...
          try {
            ...
            if (error) {
              ...
              // Forzamos la actualización del UI en el siguiente tick para evitar condiciones de carrera
              setTimeout(() => handleLoginSuccess(data.user), 10);
            }
          } catch (err) {
            console.error("Error en login:", err);
            if (errorDiv) {
              errorDiv.textContent = "Error en login: " + (err.message || String(err));
              errorDiv.hidden = false;
            }
          }
        });
      ...
      console.log("NY TRIP: login handler ya registrado, saltando registro duplicado.");
    }
  });
  ...
  function renderReservations() {
    ...
    listEl.innerHTML = state.reservations
      .map(r => `
        <div class="card">
  ...
  function renderExpenses() {
    ...
    listEl.innerHTML = state.expenses
      .map(e => `
